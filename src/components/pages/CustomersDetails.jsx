...
function CustomersDetails({ buttonText, popupContent }) {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    fetchAllCustomers();
    fetchAllCustomersname();
  }, [currentPage]);

  const fetchAllCustomers = async () => {
    setCustomersDetailsList([]);
    // setLoader(true);
    let params = {
      limit: PageSize,
      pageNo: currentPage,
    ...
    if (response?.data?.success) {
      ...
      setCustomersDetailsList(response?.data?.data?.customersDetails);
    } else {
      setCustomersDetailsList({});
      if (
  ...
  const fetchAllCustomersname = async () => {
    setCustomersDetailsData([]);
    // setLoader(true);
    let params = {
      limit: PageSize,
      pageNo: currentPage,
    ...
    const response = await getAllCustomerDetails(params);
    if (response?.data?.success) {
      console.log(response);
      setCustomersDetailsData(response?.data?.data?.Customers_Details);
    } else {
      setCustomersDetailsData([]);
      if (
        response?.response?.status == 403 ||
  ...
  const handleChange = (e) => {
    ...
    if (customersData.name == null || customersData.name == "") {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(customersData).length > 0) {
      Object.keys(customersData).map((item, index) => {
        if (item != "created_at" && item != "updated_at") {
          if (customersData?.[item] == null || customersData?.[item] == "") {
            const { name, value } = {
              name: item,
    ...
    const response = isEdit
      ? await editCustomers(customersData, customersData?.id)
      : await addNewCustomers(customersData);
    console.log(response);
    if (response?.data?.success) {
      console.log(response);
  ...
  const deleteSelectedSite = async () => {
    const response = await deleteCustomers(customersData?.id);
    console.log(response);
    if (response?.data?.success) {
      console.log(response);
      // setloginData(response?.data?.data);
  ...
  const togglePopup = () => {
    setPopUpHeaderName("Add New Customer");
                            ...
                            <div>
                                ...
                                >
                                  ...
                                  {customersDetailsData ? (
                                    Object.values(customersDetailsData)?.map(
                                      (item, index) => (
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  value={customersData.total_amount_due}
                                />
                              </div>
                            </div>
        ...
        {/* <div class="relative overflow-x-auto shadow-md sm:rounded-lg lg:mt-[1%] xl:w-[80]">
          <div class="flex flex-row justify-between p-5">
            <div>
              <span>
                Petrol Pump Name :{" "}
                <span class="text-dark">{customersData?.name}</span>{" "}
              </span>
            </div>
          </div>
        </div> */}
        <div className="flex justify-end lg:pr-[2%] mt-2">
          <button
            onClick={togglePopup}
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg lg:mt-[1%] xl:w-[80] p-5">
          ...
          {customersDetailsList?.length > 0 ? (
            ...
            <thead class="text-xs text-gray-600 border-b-2 border-[#b2b2b2] uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
              ...
              <tbody>
                {customersDetailsList.length > 0
                  ? Object.values(customersDetailsList)?.map((item, index) => (
                      <tr
                        key={index}
                        class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                      >
                        <td class="px-6 py-4">{item?.customer_details
