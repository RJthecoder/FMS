...
function DailyFuelRate({ buttonText, popupContent }) {
  const [showPopup, setShowPopup] = useState(false);
  const [dailyFuelRateList, setDailyFuelRateList] = useState([]);
  ...
  const [dailyFuelRateData, setDailyFuelRateData] = useState({});
  ...
  const fetchAllDailyFuelRate = async () => {
    setLoader(true);
    setDailyFuelRateList([]);
    // setLoader(true);
    const response = await getAllDailyFuelRate();
    console.log(response?.data);
    if (response?.data?.success) {
      setLoader(false);
      setDailyFuelRateList(response?.data?.data?.daily_fuel_rate);
      console.log(dailyFuelRateList);
    } else {
      setDailyFuelRateList([]);
      setLoader(false);
    }
  ...
  const handleChange = (e) => {
    ...
    if (dailyFuelRateData.name == null || dailyFuelRateData.name == "") {
      setErrors({ ...errors, [name]: "" });
    }
    setDailyFuelRateData({ ...dailyFuelRateData, [name]: value });
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (Object.keys(dailyFuelRateData).length > 0) {
      Object.keys(dailyFuelRateData).map((item, index) => {
        if (item != "created_at" && item != "updated_at") {
          if (dailyFuelRateData?.[item] == null || dailyFuelRateData?.[item] == "") {
            const { name, value } = {
              name: item,
    ...
    const response = isEdit
      ...
      await addNewDailyFuelRate(dailyFuelRateData);
    ...
    <Layout>
      <div>
        {loader ? (
          ...
          <div class="text-2xl font-bold text-gray-800 mb-4 mt-2">Daily Fuel Rate</div>
            <div
                              ...
                              >
                                ...
                                <div>
                                    <select
                                      ...
                                      value={dailyFuelRateData?.petrol_pump_id}
                                    >
                                  ...
                                  <div>
                                    <select
                                      ...
                                      value={dailyFuelRateData?.fuel_type_id}
                                    >
                                      {errors.fuel_type_id && (
                                        <p className="error-text">
                                          {errors.fuel_type_id}
                                  ...
                                  <div>
                                    ...
                                    <input
                                      ...
                                      placeholder="Enter Daily Fuel Rate"
                                      onChange={(e) => {
                                        handleChange(e);
                                      }}
                                      value={dailyFuelRateData.rate}
                                    />
                                  </div>
                ...
                {dailyFuelRateList?.length > 0 ? (
                  <div class="overflow-x-auto overflow-y-auto">
                    ...
                    <thead class="text-xs text-gray-600 border-b-2 border-[#b2b2b2] uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                      ...
                      <tbody>
                        {dailyFuelRateList.length > 0
                          ? Object.values(dailyFuelRateList)?.map((item, index) => (
                              <tr
                                key={index}
                              ...
                              >
                                ...
                                <td class="px-6 py-4">{Number(item?.rate)?.toFixed(2)}</td>
