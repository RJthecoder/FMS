...
function AddPetrolPump({ buttonText, popupContent }) {
  ...
  const fetchAllPetrolPump = async () => {
    ...
    if (response?.data?.success) {
      setLoader(false)
      setPetrolPumpList(response?.data?.data?.petrolPumps);
      console.log(petrolPumpList);
    } else {
      setPetrolPumpList([]);
      setLoader(false)
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const response = isEdit
      ? await editPetrolPump(petrolPumpData, petrolPumpData?.id)
      : await addNewPetrolPump(petrolPumpData);
  ...
  const deleteSelectedSite = async () => {
    setLoader(true)
    const response = await deletePetrolPump(petrolPumpData?.id);
    console.log(response);
    if (response?.data?.success) {
      console.log(response);
      // setloginData(response?.data?.data);
  ...
  const togglePopup = () => {
    setPopUpHeaderName("Add New Petrol Pump");
  ...
  return (
    ...
    <div>
      ...
      <div className="mt-4" style={{marginTop:"4rem", marginBottom:"2rem"}}>
        ...
        <div class="text-2xl font-bold text-gray-800 mb-4 mt-2">PetrolPump</div>
                            ...
                            <div className="rounded-md shadow-sm  space-y-4">
                              ...
                              <div>
                                ...
                                <input
                                  ...
                                  value={petrolPumpData.abbrevation}
                                />
                              </div>
                              <div>
                                <label htmlFor="password" className="sr-only">
          ...
          <div class="relative overflow-auto shadow-md sm:rounded-lg lg:mt-[1%] xl:w-[80]">
            {petrolPumpList?.length > 0 ? (
              ...
              <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {petrolPumpList.length > 0
                    ? Object.values(petrolPumpList)?.map((item, index) => (
                        <tr
                          key={index}
                          class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600"
                        >
                          <td class="px-6 py-4">{item?.name}</td>
                          <td class="px-6 py-4">{item?.abbrevation}</td>
                          <td class="px-6 py-4">{item?.address}</td>
                          <td class="px-6 py-4 flex gap-3">
                            <a
                              onClick={() => navigate(`/petrol-pump/${item?.id}`)}
