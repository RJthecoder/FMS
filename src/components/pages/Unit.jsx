...
function Unit({ buttonText, popupContent }) {
  ...
  const handleChange = (e) => {
    ...
    if (unitData.name == null || unitData.name == "") {
  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(unitData).length > 0) {
      Object.keys(unitData).map((item, index) => {
        if (item != "created_at" && item != "updated_at") {
          if (unitData?.[item] == null || unitData?.[item] == "") {
            const { name, value } = {
              name: item,
    ...
    delete unitData["created_at"];
    delete unitData["updated_at"];
    const response = isEdit
      ? await editUnit(unitData, unitData?.id)
      : await addNewUnit(unitData);
  ...
  const deleteSelectedSite = async () => {
    setLoader(true);
    const response = await deleteUnit(unitData?.id);
    console.log(response);
    if (response?.data?.success) {
      console.log(response);
      // setloginData(response?.data?.data);
  ...
  const togglePopup = () => {
    setPopUpHeaderName("Add New Unit Type");
      ...
      <div>
        {loader ? (
          ...
          <div className="mt-4" style={{ marginTop: "4rem", marginBottom: "2rem" }}>
            ...
            <div className="text-2xl font-bold text-gray-800 mb-4 mt-2 ">Unit</div>
            <div className=" justify-end mt-2 " style={{ marginBottom: "0.5rem" }}>
              <button
                onClick={togglePopup}
                className="bg-blue-500 hover:bg-[#0265cf] text-white font-bold py-2 px-4 rounded"
              >
                +ADD
              </button>
              </div>
                      ...
                      <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                        <div className="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                          <div className="sm:items-start">
                            <div className="mt-3 items-center sm:mt-0 sm:ml-0 sm:text-left">
                ...
                {unitList?.length > 0 ? (
                  <div>
                    <table class="w-full text-sm font-[500] text-left rtl:text-right text-gray-900 dark:text-gray-400">
                      ...
                      <tbody>
                        {unitList.length > 0
                          ? Object.values(unitList)?.map((item, index) => (
                              <tr
                                key={index}
